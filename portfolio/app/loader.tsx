...
const Loader = ({ setLoading }: { setLoading: (value: boolean) => void }) => {
  ...
  useEffect(() => {
    if (textRefs.current.some((el) => el === null)) return; // ✅ Ensure refs are not null

    const tl = gsap.timeline({
      onComplete: () => setLoading(false), // ✅ No need for setTimeout
    });
    ...
    tl.to(textRefs.current[0], { opacity: 1, y: 0, duration: 1, ease: "power3.out" })
      ...
      .to(textRefs.current.filter(Boolean), { opacity: 0, duration: 0.5, ease: "power3.inOut" }, "+=1"); // ✅ Ensures only valid refs are animated
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black text-white text-4xl font-bold z-50">
      <div className="flex flex-col items-center gap-5">
        <span
          ...
          }} // ✅ Now TypeScript will not complain
