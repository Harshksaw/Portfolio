...
export default function Orb({
  ...
  // Detect mobile and disable blob on mobile for performance
  useEffect(() => {
    const checkMobile = () => {
  ...
  useEffect(() => {
    // Keep blob on mobile but optimize performance
    ...
    try {
      ...
      // WebGL unavailable or blocked — ogl throws when getContext returns null
      return;
    }
    ...
    const update = (t: number) => {
      ...
      // Pause rendering when not visible for better performance
      if (!isVisible) {
        return;
      }
    ...
    // Use lower frame rate on mobile for better performance
    if (isMobile) {
      let frameCount = 0;
      const mobileUpdate = (t: number) => {
        rafId = requestAnimationFrame(mobileUpdate);
        ...
        // Render every other frame on mobile to improve performance
        frameCount++;
        if (frameCount % 2 !== 0) {
          return;
        }
  ...
  }, [hue, hoverIntensity, rotateOnHover, forceHoverState, isMobile, isVisible]);

  return <div ref={ctnDom} className="w-full h-full absolute -translate-x-4 md:translate-x-0" />;
}
